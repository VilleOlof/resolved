// https://github.com/mlua-rs/mlua/blob/main/mlua_derive/src/chunk/token.rs
...
fn span_pos(span: Span) -> Result<(Pos, Pos), TokenStream2> {
    ...
    if start.line == 0 || end.line == 0 {
        return Err(syn::Error::new(
            Span2::call_site(),
            "cannot retrieve span location information; resolved requires nightly Rust or stable >= 1.88",
        )
        .to_compile_error());
    }
...
impl Token {
    ...
    pub(crate) fn is_capture(&self) -> bool {
        self.attr == TokenAttr::Capture
    }

    pub(crate) fn is_itemref(&self) -> bool {
        self.attr == TokenAttr::ItemRef
    }
    ...
    pub(crate) fn retokenize(tt: TokenStream) -> Result<Tokens, TokenStream2> {
        ...
        while let Some(t) = iter.next() {
            ...
            if t.is("$") {
                ...
                {
                    tokens.push(next.attr(TokenAttr::Capture));
                } else {
                    return Err(syn::Error::new(
                        t.tree.span().into(),
                ...
                if let Some(next) = iter.next()
                    && matches!(next.tree, TokenTree::Ident(_))
                {
                    tokens.push(next.attr(TokenAttr::ItemRef));
                } else {
                    return Err(syn::Error::new(
                        t.tree.span().into(),
                        "`@` must be followed by an identifier",
