...
impl<T> ScriptResponse<T> {
    ...
    pub fn value(self) -> Option<T> {
        match self {
            Self::Err(_) => None,
            Self::UnableToReachResolve => None,
            Self::Ok {
                value,
                eval_time: _,
            } => Some(value),
...
pub struct ModuleConfig {
    /// if globals generated by execution code should be reset.
    /// This resets all globals to its init state before running the code
    pub reset_globals: bool,
    /// Globlas which always persist from the start
    pub globals: Vec<(String, rmpv::Value)>,
    /// The resolve scriting API function to check if it exists to determine if davinci resolve is still open.
    /// If not specified it will not check for it at all and instead return a module error that some value was nil
    pub function_check: Option<String>,
}
